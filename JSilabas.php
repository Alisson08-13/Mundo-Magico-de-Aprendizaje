...
<head>
  ...
  <style>
    ...
    .color-6 { background: #a7f3d0; color: #047857; --shadow-color: #047857; }
    ...
    .btn-childish img { height: 1.5em; border-radius: 8px; }
...
<body>
  ...
  <script>
    const silabas = ["ma","me","mi","mo","mu","pa","pe","pi","po","pu","ta","te","ti","to","tu","la","le","li","lo","lu","sa","se","si","so","su"];
    const coloresCSS = ["color-1","color-2","color-3","color-4","color-5","color-6"];
    const grid = document.getElementById('syllable-grid');
    const silabaObjetivoSpan = document.getElementById('silabaObjetivo');
    ...
    function crearBotones() {
      grid.innerHTML = '';
      let opciones = [silabaObjetivo];
      while (opciones.length < 8) {
        const aleatoria = silabas[Math.floor(Math.random() * silabas.length)];
        if (!opciones.includes(aleatoria)) opciones.push(aleatoria);
      }
      opciones = mezclarArray(opciones);
      opciones.forEach((s, i) => {
        const b = document.createElement('button');
        const c = coloresCSS[Math.floor(Math.random() * coloresCSS.length)];
        b.className = `btn-3d syllable-btn ${c}`;
        b.textContent = s;
        b.onclick = clickSilaba;
        b.style.animationDelay = `${i * 0.05}s`;
        grid.appendChild(b);
      });
    }

    function clickSilaba(e) {
      if (!juegoActivo) return;
      const boton = e.currentTarget;
      const clicada = boton.textContent;
      reproducirTexto(clicada);

      if (clicada === silabaObjetivo) {
        puntaje++;
    ...
    function nuevaSilaba() {
      silabaObjetivo = silabas[Math.floor(Math.random() * silabas.length)];
      silabaObjetivoSpan.textContent = silabaObjetivo;
      setTimeout(() => reproducirTexto("Busca la sílaba " + silabaObjetivo), 300);
    }

    function nuevaRonda() {
      mensajeDiv.textContent = '¡Toca la sílaba correcta!';
    ...
    function finalizar() {
      ...
      mensajeDiv.textContent = `¡Tiempo terminado! Puntaje: ${puntaje} 🎉`;
    ...
    window.onload = () => {
      setTimeout(() => {
        reproducirTexto("¡Hola! Es momento de jugar con las sílabas.");
        iniciarJuego();
      }, 500);
    };
  </script>
