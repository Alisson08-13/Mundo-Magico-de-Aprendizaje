...
<head>
    ...
    <style>
        /* 1. Importación de Fuente (Nunito) */
        ...
        /* 5. Grid de Letras (Aplicado a las vocales) */
        .letter-selector-grid {
            display: grid;
            /* Se ajustará a 5 columnas en pantallas anchas */
            grid-template-columns: repeat(auto-fit, minmax(80px, 1fr));
            gap: 1rem;
            margin-top: 1.5rem;
            margin-bottom: 2rem;
            
            /* Centrado para las 5 vocales */
            max-width: 600px;
            margin-left: auto;
            margin-right: auto;
        }
        ...
        .btn-3d:not(:active):hover {
            animation: jiggle 0.3s;
        }

        @keyframes shine {
            0% { box-shadow: 0 5px 0 0 var(--shadow-color), 0 0 4px transparent; }
            50% { box-shadow: 0 5px 0 0 var(--shadow-color), 0 0 15px var(--shadow-color); }
            100% { box-shadow: 0 5px 0 0 var(--shadow-color), 0 0 4px transparent; }
        }
        @keyframes jiggle {
        ...
        /* Colores temáticos (Se aplicarán a las 5 vocales en orden) */
        ...
        /* La regla 5n+0 no se usará al haber solo 5 botones */
        ...
        #animatedLetterDisplay {
            font-weight: 900;
            font-size: clamp(8em, 30vw, 15em);
            color: #ccc;
            transition: color 0.4s;
            user-select: none;
        }
        #animatedLetterDisplay.initial {
        ...
        .animated-letter {
            animation: boing 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards;
        }
        ...
        @media (max-width: 600px) {
            ...
            /* Ajuste para 3 vocales por fila en móvil */
            .letter-selector-grid {
        ...
        /* Ajuste extra para centrar las 5 vocales en pantallas medianas */
...
<body>
    ...
    <div class="main-container">
        ...
        <p>Toca una vocal para oír su sonido y ver la animación.</p>
        ...
        <section class="sound-display-area" id="soundDisplay">
            <span id="animatedLetterDisplay" class="initial">¡Bienvenido a las Vocales!</span>
        </section>
...
<script>
    ...
    const letterButtons = document.querySelectorAll('.letra-btn'); // Funciona con las nuevas clases
    ...
    function hablar(texto) {
        if ('speechSynthesis' in window) {
            ...
            voz.text = texto; // Texto por defecto (A, E, I, O, U se pronuncian bien)

            // Casos especiales (no afectarán a las vocales)
            ...
            else if (letraMinuscula === 'q') { voz.text = 'cu'; }
    ...
    // --- LÓGICA PRINCIPAL ---
     document.addEventListener('DOMContentLoaded', () => {
        setTimeout(() => {
            // Texto actualizado para "Vocales"
             hablar('Toca una vocal para escucharla.');
        }, 500);
        
        // Texto inicial actualizado para "Vocales"
        if(animatedLetterDisplay.classList.contains('initial')) {
             animatedLetterDisplay.textContent = '¡Bienvenido a las Vocales!';
        }
     });

    letterButtons.forEach(v => {
        v.addEventListener('click', () => {
            const letra = v.dataset.letra; // Funciona con data-letra
            ...
            // Decidir color de la letra grande
            if (vocales.includes(letra.toLowerCase())) {
                displayColor = displayColoresVocales[Math.floor(Math.random() * displayColoresVocales.length)];
            } else {
            ...
            // Efecto de color en el display (parte del diseño del Abecedario)
